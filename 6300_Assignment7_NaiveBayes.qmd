...
Answer: 0.098 is the benchmark error rate.  If we always predict the mode (0), we would be wrong 9.8% of the time. This is our baseline to compare with other models

```{r}
(169+27)/(1786+27+169+18)
```
...
Answer: our model is slighlty better than the benchmark, which means it adds some value. however we should be cautious because the improvement is minimal and the model may not generalize well to new data especially if it’s overfitting or relies on assumptions that may not hold in real world scenarios
...
Answer: 44,37% of personal in the dataset have an education level of 3 

```{r}
model$tables$Education
```
...
Answer: The probability that a customer has education level 3 given they responded positively to the personal loan marketing campaign is 15,81%

```{r}
bank_subset <- bank[bank$Education == "3",]
odds <- sum(bank_subset$Personal.Loan=="1") / sum(bank_subset$Personal.Loan=="0")
odds
```
...
Answer: the two answers reveal that, although a large portion of the banks customers have advanced degrees (44.37%), only a small percentage of them (15.82%) responded positively to personal loan ads. this suggests that highly educated customers might not be very interested in personal loans, possibly due to higher income, financial stability, or different product preferences
...
Answer: the code calculates the odds of customers having taken a personal loan, resulting in an odds ratio of 0.1046
...
Answer: the code calculates the odds of customers with advanced degrees taking a personal loan, resulting in an odds ratio of 0.1656
